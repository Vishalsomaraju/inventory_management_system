...
const Analytics = () => {
  const [stockMovement, setStockMovement] = useState({ data: [], loading: true });
  ...
  const fetchStockMovement = async (days) => {
    ...
    try {
      ...
      setStockMovement({ data: Array.isArray(res.data) ? res.data : (res.data.data || []), loading: false });
    } catch {
  ...
  const fetchPoSummary = async () => {
    ...
    try {
      ...
      setPoSummary({ data: res.data.by_status || res.data || [], loading: false });
    } catch {
  ...
  const ChartCard = ({ title, subtitle, onRefresh, loading, children, headerRight }) => (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col">
      <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-xl">
        ...
        <div className="flex items-center gap-3">
          {headerRight}
          <button onClick={onRefresh} className="p-1.5 text-gray-400 hover:text-blue-600 rounded-md transition-colors" title="Refresh">
      ...
      <div className="p-6 flex-1 min-h-[300px] relative flex justify-center items-center">
        {loading && <div className="absolute inset-0 bg-white/80 z-10 flex items-center justify-center animate-pulse"><div className="w-8 h-8 rounded-full border-4 border-blue-500 border-t-transparent animate-spin"></div></div>}
        {children}
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6">
        ...
        <p className="text-sm text-gray-500">Real-time metrics from the analytics engine</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          {stockMovement.data.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={stockMovement.data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#eee"/>
                <XAxis dataKey="date" tick={{fontSize: 12, fill: '#64748b'}} tickFormatter={(v) => new Date(v).toLocaleDateString(undefined, {month:'short', day:'numeric'})} />
                <YAxis tick={{fontSize: 12, fill: '#64748b'}} />
                <Tooltip contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
                <Legend iconType="circle" wrapperStyle={{ fontSize: '12px' }}/>
                <Line type="monotone" dataKey="total_in" name="Stock In" stroke="#3b82f6" strokeWidth={3} strokeLinecap="round" dot={false} activeDot={{r: 6}}/>
                <Line type="monotone" dataKey="total_out" name="Stock Out" stroke="#ef4444" strokeWidth={3} strokeLinecap="round" dot={false} activeDot={{r: 6}}/>
                <Line type="monotone" dataKey="net" name="Net Change" stroke="#94a3b8" strokeDasharray="5 5" strokeWidth={2} dot={false}/>
              </LineChart>
            </ResponsiveContainer>
        ...
        >
          {topProducts.data.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topProducts.data} layout="vertical" margin={{ top: 0, right: 30, left: 10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#eee"/>
                <XAxis type="number" tick={{fontSize: 12, fill: '#64748b'}}/>
                <YAxis dataKey="product_name" type="category" width={100} tick={{fontSize: 11, fill: '#64748b'}} />
                <Tooltip cursor={{fill: '#f1f5f9'}} contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
                <Bar dataKey="quantity" name="Quantity Out" fill="#3b82f6" radius={[0, 4, 4, 0]}>
        ...
        >
          <div className="h-full overflow-y-auto w-full absolute inset-0">
            {lowStock.data.length > 0 ? (
              <table className="w-full text-left text-sm border-collapse">
                <thead className="bg-gray-50/90 backdrop-blur sticky top-0 border-b border-gray-100 text-gray-600 font-semibold shadow-sm z-10">
                  <tr>
                    ...
                    <th className="py-2.5 px-4 text-center font-semibold">Deficit</th>
                    <th className="py-2.5 px-4 font-semibold text-right">Vendor</th>
                  </tr>
                </thead>
                  ...
                  {lowStock.data.map((item, i) => {
                    ...
                    return (
                      <tr key={item.id || i} className="hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4 text-center text-red-600 font-bold bg-red-50/50">{item.current_stock} <span className="font-normal text-xs text-red-400">{item.unit}</span></td>
                        <td className="py-3 px-4 text-center text-amber-600 font-bold bg-amber-50/50">-{deficit > 0 ? deficit : 0}</td>
                        <td className="py-3 px-4 text-gray-500 text-right truncate max-w-[100px]">{item.vendor?.name || item.vendor_id || '—'}</td>
                      </tr>
                    );
                  })}
                </tbody>
              ...
              <div className="flex justify-center items-center h-full text-gray-400">All stock is sufficient.</div>
            )}
          </div>
        </ChartCard>
